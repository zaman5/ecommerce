/**
 * Adds the marketplace departments (Electronics, Fashion, Collectibles and art,
 * Sports, Health and beauty, Home and garden) that the navbar links to.
 *
 * Unlike `seed.js` this script is ADDITIVE — it never deletes. Categories and
 * products are upserted by slug, so the school-supply catalogue stays exactly
 * as it is and re-running this is safe.
 *
...
// Every ID below is checked to return 200. Unsplash retires photo IDs over
// time, so if an image goes blank swap the ID and re-run.
const photos = {
...
// Order here matches the order of the navbar category strip.
...
// Reusable colourways. Keep the names customer-facing — they are what gets
// printed on the order and picked in the warehouse.
const C = {
...
// Colourways offered per department. Anything whose colour is not a customer
// choice (consumables, media, a specific alloy wheel) is listed in NO_COLOUR
// below and keeps an empty list, which hides the picker on its page.
...
async function run() {
  ...
  const cats = await Category.find({ slug: { $in: categoriesSeed.map((c) => c.slug) } });
